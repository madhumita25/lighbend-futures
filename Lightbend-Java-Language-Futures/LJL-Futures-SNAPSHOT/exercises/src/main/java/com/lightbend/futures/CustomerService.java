...
class CustomerService {
    ...
    CompletableFuture<Optional<String>> getCustomerFirstName(UUID customerId) {
        return customerRepository.getCustomer(customerId).thenApply(c -> c.map(Customer::getFirstName));
    }

    CompletableFuture<Optional<String>> getCustomerLastName(UUID customerId) {
        return customerRepository.getCustomer(customerId).thenApply(c -> c.map(Customer::getLastName));
    }

    CompletableFuture<Optional<String>> getCustomerAddress(UUID customerId) {
        return customerRepository.getCustomer(customerId).thenApply(c -> c.map(Customer::getAddress));
    }

    CompletableFuture<Optional<String>> getCustomerPhoneNumber(UUID customerId) {
        return customerRepository.getCustomer(customerId).thenApply(c -> c.map(Customer::getPhoneNumber));
    }
